...
cols = [x for x in range(w) if any(img.getpixel((x,y))[3] > 128 for y in range(48))]
ranges = []
for c in cols:
...
print("Ranges for y in [0, 48):", ranges)

# Let's also check y ranges for each object found:
for i, (startX, endX) in enumerate(ranges):
    ys = [y for y in range(48) for x in range(startX, endX+1) if img.getpixel((x,y))[3] > 128]
    if ys:
